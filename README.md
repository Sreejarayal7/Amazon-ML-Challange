...
Install the required libraries: The project includes a cell in main.ipynb to install all dependencies. Run the first cell in the notebook to install everything.
...
Download Data: Untar the train.csv and test.csv zip files. The notebook contains cells to download all the necessary product images into the images/ folder.

Generate Features: Run the cells in the notebook to:
...
Train the Final Model: Run the final cell, which performs memory-safe K-Fold training on the ensemble of LightGBM, XGBoost, and CatBoost.

Generate Submission: The script will automatically create the test_out.csv file in the root directory, ready for upload.
